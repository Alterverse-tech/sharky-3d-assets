...
4. **Render one complete asset confirmation table.** Pass the complete proposal to `render_asset_sourcing_board`. In MCP-Apps-capable hosts this opens a Widget with exactly these columns and this order: 角色/实体、资产需求、选项、候选方案（点击预览）、模型来源、动作、触发场景、动作来源、复用状态、当前选择. Each 资产需求 is one single-choice group. Any real reusable candidate name with a safe HTTP/HTTPS `previewUrl` is clickable and opens that external preview page; absent candidates have no placeholder row. The Widget does not embed preview iframes or request fullscreen. In plain hosts, render the same structured proposal as a markdown table:

   | 角色/实体 | 资产需求 | 选项 | 候选方案（点击预览） | 模型来源 | 动作 | 触发场景 | 动作来源 | 复用状态 | 当前选择 |
   |---|---|---|---|---|---|---|---|---|---|
   | 主角骑士（key） | 本体模型 | A | 生成原创主角骑士 | 新生成 · Gemini Reference → Tripo | — | 玩家进入城堡 | — | 原创主模型 | ☑ 推荐 |
   | 〃 | 本体模型 | B | [Asset Center 候选：Rusty Knight](https://example.invalid/preview) ↗ | Asset Center 静态 GLB | — | 玩家进入城堡 | — | 候选复用 | ☐ |
   | 主角骑士（key） | 移动动作 | A | 生成 `walk` 动作 GLB | 沿用已选主角骑士本体 | walk | 第三人称移动探索 | `preset:biped:walk` | 与新本体共用骨骼 | ☑ 推荐 |
