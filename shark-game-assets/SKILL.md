...
- When the workspace has a frozen `animation-plan.json`, the synchronizer also maintains `animation-plan-progress.md` beside it: the confirmed action requirements table plus a live per-row 状态 column (⬜ pending / 🔄 running % / ✅ done / ❌ failed; procedural rows show ✅ 运行时), atomically overwritten whenever derived status changes. Point the user at this file for text-form progress; never hand-edit it.
